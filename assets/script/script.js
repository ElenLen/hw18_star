...
// const supHeroJSON = JSON.stringify(arr);
// console.log(supHeroJSON);
// console.log(typeof supHeroJSON); //"string"

// const supHeroObject = JSON.parse(supHeroJSON);
// console.log(supHeroObject);
// console.log(typeof supHeroObject); //"object"
// console.log(supHeroObject[0].name);
...
const createCard = (obj) => {
  ...
  div.innerHTML = `
      ...
      <p class="universe">
      <span class="cardTitle">Вселенная: </span>${obj.universe}</p>
      <p class="alterego">
      <span class="cardTitle">Альтерэго: </span>${obj.alterego}</p>
      <p class="occupation"><span class="cardTitle">Занятие: </span>${obj.occupation}</p>
      <p class="friends"><span class="cardTitle">Друзья: </span>${obj.friends}</p>
      <p class="superpowers"><span class="cardTitle">Сверхспособности: </span>${obj.superpowers}</p>
      <img
        src="${obj.url}"
        alt="${obj.name}" class="img">
      <p class="info"><span class="cardTitle">Информация: </span>${obj.info}</p>
...
arr.forEach((element) => {
  ...
  // удочеряем лист и передаем в него дом
  list.appendChild(divVDom);
});

// обработчик события на родителе при клике на дочерних
list.addEventListener("click", (event) => {
  // на какой объект нажали
